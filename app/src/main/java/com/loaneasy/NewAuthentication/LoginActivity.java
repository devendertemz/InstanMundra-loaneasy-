...
import com.loaneasy.login_signup.VerifyOtp;
...
public class LoginActivity extends AppCompatActivity implements NewUserLoginPresenter.UserLoginView, GoogleApiClient.OnConnectionFailedListener {

    TextView Login, tvVerifyTextRed;
    EditText etPhoneNumber, etPasswordLayout;

    NewUserLoginPresenter presenter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Login = findViewById(R.id.btPhoneNoSubmit);
        presenter = new NewUserLoginPresenter(this);

        tvVerifyTextRed = findViewById(R.id.tvVerifyTextRedd);
        etPasswordLayout = findViewById(R.id.etPassword);
        ...
        tvVerifyTextRed.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                Intent in = new Intent(LoginActivity.this, ForgetPasswordActivity.class);
                in.putExtra("key", "Forget");
                startActivity(in);
    ...
    public void onUserLoginSuccess(LoginRepo response, String message) {
        ...
        if (message.equalsIgnoreCase("ok")) {
            if (response.getStatus() == true) {
                    ...
                    if (response.getResponse().getProfileCompleted().equals("1")) {

                        sharedPreference.setSignFlag(3);

                        sharedPreference.setUserId(response.getResponse().getUserId());

                        sharedPreference.setUserPhoneNo(response.getResponse().getPhoneNo());
                    //    sharedPreference.setLocation(response.getResponse().getLocation());
                        sharedPreference.setUserSocialName(""+response.getResponse().getFullName());

                       // sharedPreference.setUserCity(response.getResponse().getLocation());
                        sharedPreference.setUserEmail(response.getResponse().getEmail());
                        Intent in = new Intent(LoginActivity.this, HomeActivity.class);
                        startActivity(in);
                        finish();

                     /*   String profilePic = "";
                        if (!response.getResponse()..optString("profile_pic").isEmpty())
                ...
                */
                   ...
                   /* else if (response.getResponse().getExistingCustomer().equals("1")){
                        ...
                        //sharedPreference.setUserEmail(response.getResponse().get);
                        Log.d("TAG111","uid---"+sharedPreference.getUserId());
...
//                sharedPreference.setExistCustomerFlag(3);
            ...
            // Toast.makeText(LoginActivity.this, response.getResponse().getUserId()+"", Toast.LENGTH_SHORT).show();
    ...
    public void DoLogin() {
        ...
        String pass = etPasswordLayout.getText().toString().trim();
        if (email.isEmpty()) {
            ...
            presenter.UserLogin(LoginActivity.this, new LoginRequest(email, pass));
